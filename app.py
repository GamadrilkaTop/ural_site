...
def catalog_railway():
    ...
    products = conn.execute("""
        ...
        WHERE categories.name = ? AND products.is_published = 1
        ORDER BY products.id
    """, ("ЖД Запчасти",)).fetchall()
    conn.close()
    return render_template('catalog/railway.html', active='catalog', products=products)
...
def init_db():
    ...
    # Дополнительные поля для товаров в существующей базе
    ...
    if cur.fetchone()[0] == 0:
        cur.executemany(
            "INSERT INTO products (category_id, name, description, material, weight, sku) VALUES (?, ?, ?, ?, ?, ?)",
            [
                (1, "Съёмник «СМУГ»", "Съёмник муфты упругой гибкой для обслуживания пассажирских вагонов.", "Сталь 45", "2.4.93.255", "24.93.255"),
                (2, "Муфта МППГ-02", "Муфта предохранительная для генератора пассажирского вагона.", "Сталь 40Х", "4,2 кг", "94.03.245"),
                (3, "Гайка крайняя", "Сертифицированная запасная часть для пассажирских вагонов.", "Сталь 40Х", "", "34.33.254"),
                (3, "Гайка шпинтона", "Запасная часть для пассажирских вагонов.", "Сталь 45", "", "30.21.103"),
                (3, "Шайба стопорная", "Сертифицированная запасная часть.", "Сталь 65Г", "", "34.33.255"),
                (3, "Болт крепления", "Крепёжная деталь для узлов пассажирских вагонов.", "Сталь 40Х", "", "30.22.141"),
                (3, "Втулка распорная", "Деталь для узлов пассажирского вагона.", "Сталь 45", "", "34.33.301"),
                (3, "Кольцо уплотнительное", "Уплотнительная деталь для узлов пассажирских вагонов.", "Резина МБС", "", "30.21.200"),
                (3, "Планка стопорная", "Стопорная деталь для узлов пассажирских вагонов.", "Сталь 08кп", "", "34.33.260"),
                (3, "Шплинт", "Крепёжная деталь для фиксации соединений.", "Сталь 10", "", "30.22.055"),
                (3, "Прокладка регулировочная", "Регулировочная деталь для узлов пассажирских вагонов.", "Сталь 08кп", "", "34.33.280"),
                (3, "Ось шарнира", "Деталь шарнирного соединения.", "Сталь 40Х", "", "30.21.145"),
            ]
        )


    railway_products = [
        ("Гайка крайняя", "34.33.254", "Сталь 40Х"),
        ("Гайка шпинтона", "30.21.103", "Сталь 45"),
        ("Шайба стопорная", "34.33.255", "Сталь 65Г"),
        ("Болт крепления", "30.22.141", "Сталь 40Х"),
        ("Втулка распорная", "34.33.301", "Сталь 45"),
        ("Кольцо уплотнительное", "30.21.200", "Резина МБС"),
        ("Планка стопорная", "34.33.260", "Сталь 08кп"),
        ("Шплинт", "30.22.055", "Сталь 10"),
        ("Прокладка регулировочная", "34.33.280", "Сталь 08кп"),
        ("Ось шарнира", "30.21.145", "Сталь 40Х"),
    ]
    cur.execute("SELECT id FROM categories WHERE name = ?", ("ЖД Запчасти",))
    railway_category = cur.fetchone()
    railway_category_id = railway_category[0] if railway_category else 3
    for name, sku, material in railway_products:
        cur.execute("SELECT id FROM products WHERE sku = ?", (sku,))
        if cur.fetchone() is None:
            cur.execute(
                "INSERT INTO products (category_id, name, description, material, weight, sku, in_stock, is_published) VALUES (?, ?, ?, ?, ?, ?, 1, 1)",
                (railway_category_id, name, "Сертифицированная запасная часть для пассажирских вагонов.", material, "", sku)
            )
...
def admin_product_add():
    ...
    if request.method == 'POST':
        conn.execute(
            "INSERT INTO products (category_id, name, description, material, weight, sku, in_stock, is_published) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (
...
def admin_product_edit(item_id):
    ...
    if request.method == 'POST':
        conn.execute(
            "UPDATE products SET category_id=?, name=?, description=?, material=?, weight=?, sku=?, in_stock=?, is_published=? WHERE id=?",
            (
                request.form.get('category_id') or None,
                request.form.get('name', '').strip(),
                request.form.get('description', '').strip(),
